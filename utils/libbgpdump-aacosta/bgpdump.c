...
void process(BGPDUMP_ENTRY *entry) {
	...
	case BGPDUMP_TYPE_MRTD_TABLE_DUMP:
		...
		printf("VIEW: %d\n",entry->body.mrtd_table_dump.view);
	    	// printf("SEQUENCE: %d; ",entry->body.mrtd_table_dump.sequence);
	    	printf("PREFIX: %s/%d; ",prefix_str,entry->body.mrtd_table_dump.mask);
	    	printf("FROM:");
		switch(entry->subtype)
		{
#ifdef BGPDUMP_HAVE_IPV6
		...
		//printf("FROM: %s AS%d; ",inet_ntoa(entry->body.mrtd_table_dump.peer_ip.v4_addr),entry->body.mrtd_table_dump.peer_as);
...
void show_attr(attributes_t *attr) {
    ...
    if(attr != NULL) {
	    ...
	    if( (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_AS_PATH) ) !=0)		
		    printf("ASPATH: %s; ",attr->aspath->str);
